<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Budgets</h1>
    </div>

    <div class="rounded border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
      <div class="text-sm font-semibold text-gray-900 dark:text-gray-100">Create Budget</div>
      <form class="mt-3 grid gap-3 md:grid-cols-5" @submit.prevent="create">
        <select v-model="form.category" class="w-full rounded border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-100">
        ...
        <input
          ...
          @input="onAmountInput"
        />
        <input v-model.number="form.month" type="number" min="1" max="12" placeholder="Month" class="w-full rounded border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-100" />
        <input v-model.number="form.year" type="number" placeholder="Year" class="w-full rounded border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-100" />
    ...
    <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      <div v-for="b in budgetStore.budgets" :key="b.id" class="space-y-2">
        <BudgetCard :budget="b" />
        <button class="rounded border border-gray-200 px-3 py-1 text-sm text-gray-700 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-300 dark:hover:bg-gray-800" type="button" @click="remove(b.id)">
          Delete
        </button>
      </div>
