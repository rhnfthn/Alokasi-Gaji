<template>
  <div class="rounded border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
    ...
    <div v-if="goal.deadline" class="mt-2 space-y-1">
      ...
      <div v-if="timeProgress !== null" class="h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-800">
        <div class="h-full rounded-full bg-gray-900 dark:bg-gray-100" :style="{ width: (Math.round(timeProgress * 100)) + '%' }" />
      </div>
    </div>
  </div>
</template>
